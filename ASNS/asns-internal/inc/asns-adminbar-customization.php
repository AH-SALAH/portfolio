...
@package asns theme
	======================================
		ASNS-INTERNAL ADMINBAR FUNCTIONS
	======================================
*/
...
// Change the WP Logo Icon within the My Sites Menu
    ...
    foreach ( (array) $wp_admin_bar->user->blogs as $blog ) { 
        ...
        $blavatar = '<img src="' . get_template_directory_uri() . '/img/asns-11-icon.png" style="vertical-align:middle;margin:5px 5px 0 0" alt="' . esc_attr__( 'Blog avatar','asns-internal' ) . '" width="16" height="16" class="blavatar"/>';  
...
// add an icon & visit link
		...
		$wp_admin_bar->add_menu( array(
			'id' => 'Visit',
			'title' => '<img src="'.get_template_directory_uri().'/img/asns-11-icon.png" style="vertical-align:middle;margin-right:5px;" alt="Visit Site" title="Visit Site" />'.__('Visit Site','asns-internal').' >>',
			'href' => site_url(),
...
// add new menus
        ...
        $wp_admin_bar->add_menu( array(  
            ...
            'title'     => '<img src="'.get_theme_root_uri().'/asns/img/google.gif" style="width:60px;height:32px;" alt="google" title="'.__('Go to GOOGLE','asns-internal').'" />', /* Set the link title */  
...
// Add a Menu to the Theme Editor for Multisite and Standalone WordPress
    ...
    if ( function_exists('is_multisite') && is_multisite() ) {  
        $wp_admin_bar->add_menu( array(  
        	'parent' => 'top-secondary',
            'id' => 'theme-editor',  
            'title' => '<span class="dashicons-before dashicons-admin-generic"></span> '.__('Edit Theme','asns-internal').'',  
...
$date = date('d-m');
 ...
 if($curlang =="ja") {
  ...
  switch($date) {
    case '01-01':
      $message = ' '.__('新明けおめでとう~','asns-internal').' ';
    break;
 
    case '25-12':
      $message = ' '.__('メリークリスマス','asns-internal').' ';
    break;
 
    default:
      $message = ' '.__('ようこそ','asns-internal').' ';
  }
  return $message;
  ...
  switch($date) {
    case '01-01':
      $message = ' '.__('Happy New Years','asns-internal').' ';
    break;
 
    case '25-12':
      $message = ' '.__('Merry Christmas','asns-internal').' ';
    break;
 
    default:
      $message = ' '.__('Welcome','asns-internal').' ';
  }
  return $message;
...
function admin_bar_account_howdy_message() {
    ...
    $howdy  = public_holiday() . sprintf( ', %1$s', $current_user->display_name );
    ...
    $wp_admin_bar->add_menu( array(
        ...
        'meta'      => array(
            'class'     => $class,
            'title'     => __('My Account','asns-internal'),
        ),
    ) );
}
